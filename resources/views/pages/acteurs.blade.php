            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Gestion des acteurs</h4>

                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item active">Connexions > Acteurs</li>
                            </ol>
                        </div>
            ...
            <div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <form class="tablelist-form" action="{{route('acteur.create')}}" method="POST">
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="nom_ac" class="form-label">Acteur</label>
                                    <input type="text" id="nom_acteur" name="nom_acteur" maxlength="255" class="form-control" placeholder="Enter le nom de l'acteur" required />
                                    <div class="invalid-feedback">Entrer le nom de l'acteur.</div>
                                </div>
                                <div>
                                    <label for="dom" class="form-label">Domaine</label>
                                    <select class="form-control" name="choices[]" id="choices" multiple required>
                                        @foreach ($domaines as $choice)
                                        <option value="{{ $choice->id }}" >{{ strtoupper($choice->nom_domaine) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <br>
            ...
            <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <form class="tablelist-form" action="{{route('acteur.update')}}" method="POST">
                            @csrf
                            <div class="modal-body">
                                <input type="text" id="id" name="id" hidden>
                                <div class="mb-3">
                                    <label for="nom_ac" class="form-label">Acteur</label>
                                    <input type="text" id="nom_acteur2" maxlength="255" name="nom_acteur2" class="form-control" placeholder="Enter le nom de l'acteur" required />
                                    <div class="invalid-feedback">Entrer le nom de l'acteur.</div>
                                </div>
                                <div>
                                    <label for="dom" class="form-label">Domaine</label>
                                    <select class="form-control" name="choices2[]" id="choices2" multiple required>
                                        @foreach ($domaines as $choice)
                                        <option value="{{ $choice->id }}" >{{ strtoupper($choice->nom_domaine) }}</option>
                                        @endforeach
                                    </select>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4>Télécharger les acteurs en fichier :</h4>
                        ...
                        <div class="modal-body">
                            <div class="mt-2 text-center">
                                ...
                                <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
                                    <h4>Êtes vous sûr ?</h4>
                                    <p class="text-muted mx-4 mb-0">Êtes vous sûr de vouloir supprimer cet acteur ?</p>
                                </div>
                            </div>
                            <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                                <button type="button" class="btn w-sm btn-light" data-bs-dismiss="modal">Annuler</button>
                                <form action="{{route('acteur.delete')}}" method="POST">
                                    @csrf
...
@section("script")
    <script>
        ...
        updateActeur=function(acteur){
            let domaines=[]
            for(const key in acteur.domaines)
            {
                domaines.push(acteur.domaines[key].id);
            }
            console.log(domaines)
            selectElement=document.getElementById("choices2");
            for (const option of selectElement.options) {
                const optionId = parseInt(option.value);
                const isSelected=domaines.includes(optionId);
                option.selected=isSelected
            }

            document.getElementById("nom_acteur2").value=acteur.nom_acteur;
