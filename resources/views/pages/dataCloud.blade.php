  ...
  width: max-content;
    ...
    <div class="page-content">
        ...
        @endif
                        ...
                        <div class="card">
                            ...
                            <div class="card-header">
                                ...
                                <div id="items-list" class="table-responsive table-card mt-3 mb-1">
                                    @include("pages.dataCloudpartial")
                                </div>
            ...
            <div class="modal fade" id="saisieAuto" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <form class="tablelist-form" action="{{route('formulaire.reouvrir')}}" method="POST">
                            @csrf
                            <div class="modal-body">
                                <div>
                                    <label for="dom" class="form-label">Acteur</label>
                                    <select class="form-control choices-single w-100" name="acteur" id="acteur2" required >
                                        <option value="">Sélectionnez un opérateur</option>
                                        @foreach ($acteurs as $acteur )
                                            <option class="" data-tokens="{{$acteur->nom_acteur}}" value="{{$acteur->id}}">{{ strtoupper($acteur->nom_acteur) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <br>
                                ...
                                <div>
                                    ...
                                    <select id="année" required class="form-control" name="annee" required  >
                                        <option value=""> Année</option>
                                        @for($i=date('Y');$i>=2014;$i--)
                                           <option value="{{$i}}">{{$i}}</option>
                                        @endfor
                                ...
                                <div>
                                    ...
                                    <select id="periodicite2" required class="form-control" name="periodicite" required disabled >
                                    </select>
                                </div>
                            </div>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="form">
                            ...
                            <div id="comment" class="form-group mx-4 pt-2 pb-3">
                                <label class="text-primary fw-bold" for="commentaires">Commentaires :</label>
                                <textarea class="form-control" id="commentaires" disabled rows="4"></textarea>
                            </div>
                            <div id="dataButton" class="m-4 d-flex justify-content-end">
...
@section('script')
   <script>
    showCom= function(comm,operateur)
    {   $("#imnt").empty()
        if(comm.length===0)
        ...
        for(i=0;i<comm.length;i++)
        {
            ...
            if(comm[i].type==="admin")
                ...
                {align="text-start";
                template.find("#source").text(operateur);
            }
        ...
        handleClickk=function(form)
            ...
            if(form.updated_at)
            {
                ...
                $("#commentaires").val(form.commentaire)
